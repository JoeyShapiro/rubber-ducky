...
| 2026-09-14 | mobile | Quests.svelte's root carries `mt-2` (spacing below Notes in the desktop right-column) which read as a stray gap at the top once Quests became its own full-screen mobile drawer. `app.css`'s active-`[data-screen]` mobile rule now resets `margin: 0 !important` - a reminder that any component styled for its desktop position needs checking against the mobile full-screen case too. |
