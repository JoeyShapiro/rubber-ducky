...
| 2026-09-15 | auth | **`POST /login` is now rate-limited** (`$lib/rateLimit.ts`) - 5 failed attempts per client address locks that address out for 5 minutes; a successful login clears its count immediately rather than waiting out the window. Checked first, before the request body is even parsed, so a blocked address doesn't cost an Argon2id verify either. Plain in-memory `Map`, not a store - this app is one Bun process with no horizontal scaling, so there is nothing else that would need to agree on the counters, and losing them on restart is a fine tradeoff. Responds `429` with a human `Retry-After` message (`"try again in 5m"`) and a real `Retry-After` header; the login page shows the server's message rather than a generic one. **The address it keys on (`event.getClientAddress()`) is only as trustworthy as `ADDRESS_HEADER` is configured** - unset (the default), it reads the raw socket peer, which behind the Cloudflare Tunnel is `cloudflared` itself for every request, not the real visitor - meaning every visitor would currently share one bucket. Same shape of gap as `PROTOCOL_HEADER` (2026-09-15, below): safe to set `ADDRESS_HEADER=x-forwarded-for` given the tunnel is the only way in, not set here since it's a deployment-config choice rather than an application one. For a single-account app a shared bucket isn't a real problem either way - there is only supposed to be one legitimate user landing in it. |
